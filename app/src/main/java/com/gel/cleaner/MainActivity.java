...
private void showSettingsDialog() {
    ...
    // In REMOTE DEVICE MODE the gear edits the CUSTOMER iDoctor,
    // never this technician installation.
    if (GELRemoteTargetManager.isRemoteMode(this)) {
        GELRemoteSettingsController.show(this);
        return;
    }
...
private void changeLang(String code) {
    ...
    if (GELRemoteTargetManager.isRemoteMode(this)) {
        ...
        payload.put("language", code);

        GELActionRouter.route(
                this,
                "SET_LANGUAGE",
                payload,
                null
...
GELActionRouter.blockUnsupportedRemote(
        this,
        AppLang.isGreek(this)
                ? "GEL Device Diagnostics — Remote Labs"
                : "GEL Device Diagnostics — Remote Labs"
);
return;
}
    ...
    bind(R.id.btnReturnAndroid, () -> {
        ...
        if (GELRemoteTargetManager.isRemoteMode(this)) {
            GELActionRouter.blockUnsupportedRemote(
                    this,
                    AppLang.isGreek(this)
                            ? "Αλλαγή Android / Apple mode"
                            : "Android / Apple mode switch"
            );
            return;
        }
